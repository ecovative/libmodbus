...
/* Table of CRC values */
static const uint16_t table_crc_hi[] = {
...
static const uint16_t table_crc_lo[] = {
...
/* RP2350 Platform-specific time functions */
#ifdef PICO_SDK
...
uint64_t modbus_time_diff_us(struct timeval *start, struct timeval *end) {
    ...
    uint64_t end_us = (uint64_t)end->tv_sec * 1000000 + end->tv_usec;
...
/* Get the timeout interval between two consecutive bytes of the same message */
...
/* Get the timeout interval used by a server to wait for a request from a client */
...
static unsigned int compute_meta_length_after_function(int function, msg_type_t msg_type) {
    int length;
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        if (ctx->indication_timeout.tv_sec == 0 && ctx->indication_timeout.tv_usec == 0) {
            /* By default, the timeout of an indication is 0 (unlimited) */
            p_tv = NULL;
        } else {
...
static int check_confirmation(modbus_t *ctx, uint8_t *req, uint8_t *rsp, int rsp_length) {
    ...
    rsp_length_computed = compute_meta_length_after_function(function, MSG_CONFIRMATION);
    rsp_length_computed += offset + ctx->backend->checksum_length + 1;
...
/* Reads the boolean status of bits and sets the array elements
   in the destination to 1 or 0. */
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest) {
    int rc;

    if (ctx == NULL || nb > MODBUS_MAX_READ_BITS) {
        errno = EINVAL;
        return -1;
    }
...
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest) {
    ...
    if (ctx == NULL || nb > MODBUS_MAX_READ_REGISTERS) {
        errno = EINVAL;
        return -1;
    }
...
int modbus_write_bit(modbus_t *ctx, int addr, int status) {
    ...
    req_length = ctx->backend->build_request_basis(ctx, MODBUS_FC_WRITE_SINGLE_COIL, addr, status ? 0xFF00 : 0, req);
...
int modbus_write_register(modbus_t *ctx, int addr, const uint16_t value) {
    ...
    req_length = ctx->backend->build_request_basis(ctx, MODBUS_FC_WRITE_SINGLE_REGISTER, addr, (int)value, req);
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src) {
    ...
    if (ctx == NULL || nb > MODBUS_MAX_WRITE_BITS) {
        errno = EINVAL;
        return -1;
    }
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src) {
    ...
    if (ctx == NULL || nb > MODBUS_MAX_WRITE_REGISTERS) {
        errno = EINVAL;
        return -1;
    }
...
int modbus_mapping_free(modbus_mapping_t *mb_mapping) {
    if (mb_mapping == NULL) {
        return -1;
    }
...
modbus_mapping_t *modbus_mapping_new(int nb_bits,
    ...
    return modbus_mapping_new_start_address(0,
                                           nb_bits,
                                           0,
                                           nb_input_bits,
                                           0,
...
size_t strlcpy(char *dest, const char *src, size_t dest_size) {
    ...
    if (dest_size > 0) {
        size_t len = (ret >= dest_size) ? dest_size - 1 : ret;
        memcpy(dest, src, len);
        dest[len] = '\0';
    }
    return ret;
...
static void _sleep_response_timeout(modbus_t *ctx) {
    /* Response timeout is always set, even if it's zero */
#ifdef PICO_SDK
    /* RP2350 implementation */
    uint32_t sleep_time_us = ctx->response_timeout.tv_sec * 1000000 + ctx->response_timeout.tv_usec;
    if (sleep_time_us > 0) {
        sleep_us(sleep_time_us);
    }
#else
    /* POSIX/Windows implementation */
#ifdef _WIN32
...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
    return _modbus_receive_msg(ctx, msg, msg_type);
}
    ...
    case MODBUS_FC_MASK_WRITE_REGISTER: {
        ...
        if (mapping_address < 0 || mapping_address >= mb_mapping->nb_registers) {
            rsp_length = response_exception(ctx,
                                           ...
                                           rsp,
                                           0,
                                           "Illegal data address 0x%0X in mask_write_register\n",
                                           address);
        } else {
            ...
            mb_mapping->tab_registers[mapping_address] =
                (mb_mapping->tab_registers[mapping_address] & and_mask) | (or_mask & (~and_mask));
            memcpy(rsp, req, req_length);
            rsp_length = req_length;
        }
    } break;
    case MODBUS_FC_WRITE_AND_READ_REGISTERS: {
        ...
        if (nb_write < 1 || MODBUS_MAX_WR_WRITE_REGISTERS < nb_write || nb < 1 || MODBUS_MAX_WR_READ_REGISTERS < nb || nb_write_bytes != nb_write * 2) {
            rsp_length = response_exception(ctx,
                                           ...
                                           MODBUS_MAX_WR_WRITE_REGISTERS,
                                           MODBUS_MAX_WR_READ_REGISTERS);
        } else if (mapping_address < 0 || (mapping_address + nb) > mb_mapping->nb_registers || mapping_address_write < 0 || (mapping_address_write + nb_write) > mb_mapping->nb_registers) {
            rsp_length = response_exception(ctx,
                                           &sft,
                                           MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS,
                                           rsp,
